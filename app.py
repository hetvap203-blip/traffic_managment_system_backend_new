...
def get_stats():
    update_logic() 
    # Fetch live tickets directly from Database
...
def toggle_accident():
    ...
    if current_state["accident_mode"]:
        ...
        database.add_ticket(ticket_id, "CRITICAL: Collision Detected", "CRITICAL", f"Global Lockdown")
...
def chat():
    ...
    if "camera" in user_msg and ("offline" in user_msg or "black" in user_msg):
        ...
        reply = f"🎫 TICKETING: There are currently {len(database.get_active_tickets())} active incidents in the database."
